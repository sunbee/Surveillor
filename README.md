...
<img width="1385" alt="dashnr2" src="https://user-images.githubusercontent.com/5471571/150048222-3f362a48-8e3f-4d9c-b275-9fba9dcbd10c.png">
...
1. I wanted to deploy the app as a docker container. Docker is the current standard for how apps for web and mobile are deployed, at least in the linux world. Once an app is dockerized, deploying anywhere is as simple as pulling the docker image from a repository with `docker pull` and spinning up a container with `docker run`. In only two commands at the terminal prompt, the app is deployed. To create an image, I needed a base image as the starting point and this proved to be the first challenge. Many images proved unsuitable for the ARM architecture of the Raspberry Pi's microprocessor. Even when a base image was compatible with the Pi's ARMv7, it would then reject the Python libraries for scientific computation that I needed for my app. The process of creating image from Dockerfile containing the requirements manifest would stall or throw incomprehensible errors arising out of incompatibilities buried deep in the dependency tree. What worked was an iterative process of experimentation. 
    - To start with, I found a suitable base image by trial-end-error and with a little help from stackoverflow.com. 
    - Several iterations followed in which I experimented with installing one library at a time via requirements manifest, winnowing down the list to those that could be successfully installed in this manner.
    - In subsequent iterations, I tried installing the remaining libraries manually by launching an interactive shell with the docker container using `docker exec -it` to debug the installation process from within. With successful installation in this way, I had more options - create an intermediate image from the running container instance using `docker commit` or update the Dockerfile and include the `pip install` commands. I tried both and found the latter approach simpler and opted for that.
